...
export function animate(): void {
  ...
  // Spin the windmill blade groups. Rotation is around the group's local Z
  // axis, which is (after the farm's Y rotation) still a horizontal axis
  // aligned with the windmill's facing direction — correct for a vertical
  // sail wheel. dt-scaled so the speed is framerate-independent.
  const windmillSpin = dt * 1.2;
  for (const blades of animatedWindmills) {
    blades.rotation.z -= windmillSpin;
  }

  // Drive each tractor around its closed path. Positions are computed in
  // farm-local space (the tractor is a child of the rotated+scaled farm
  // group), and the heading is derived from the path tangent so we don't
  // need any world-space lookAt.
  ...
  for (const tractor of animatedTractors) {
    ...
    // Gentle engine bob (in farm-local units; scaled by the farm group).
  ...
  // Minecarts looping around circular rails in brickyard tiles. Because
  // the cart lives inside a rotated+scaled cluster group, we set its
  // local transform (position + rotation.y) directly instead of using
  // world-space lookAt — simpler and cheaper.
  for (const cart of animatedMinecarts) {
    ...
    // Gentle bob to simulate rickety rails.
    cart.position.y = 0.25 * Math.sin(nowMs * 0.018 + d.offset);
    // Circle tangent direction: +Z forward maps to rotation.y = -angle.
